...
class DatasetLoader(Dataset):
    def __init__(self, args, split_set):
        ...
        # 加载数据的比例（data_fraction）
        if hasattr(args, 'data_fraction') and args.data_fraction < 1.0:
            # 计算需要使用的样本数量
            total_size = len(pieces)
            subset_size = int(args.data_fraction * total_size)
            print(f"Loading {args.data_fraction*100}% of the dataset, which is {subset_size} samples.")
            pieces = random.sample(pieces, subset_size)  # 随机抽取样本
    ...
    def __getitem__(self, idx):
        ...
        # 确保 token 是二维张量
        if len(token.shape) == 1:  # 如果是 1D 张量
            token = token.unsqueeze(0)
        elif len(token.shape) == 0:  # 如果是标量
            token = token.unsqueeze(0).unsqueeze(1)

        # 设置最大特征大小
        max_feature_size = 65

        # 填充或截断
        if token.size(1) < max_feature_size:
            padding = torch.zeros((token.size(0), max_feature_size - token.size(1)))
            token = torch.cat([token, padding], dim=1)
        elif token.size(1) > max_feature_size:
            token = token[:, :max_feature_size]
