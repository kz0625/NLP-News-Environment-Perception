...
def get_cosine(post, date2recent_news_idx, all_news_vec, post_vec, dataset, split_name, days):
    ...
    # 目标路径不存在则创建
    output_dir = './data/{}/'.format(dataset)
...
if __name__ == "__main__":
    ...
    train = json.load(open('../../dataset/{}/post/train.json'.format(dataset), 'r', encoding='utf-8'))
    val = json.load(open('../../dataset/{}/post/val.json'.format(dataset), 'r', encoding='utf-8'))
    test = json.load(open('../../dataset/{}/post/test.json'.format(dataset), 'r', encoding='utf-8'))
    news = json.load(open('../../dataset/{}/news/news.json'.format(dataset), 'r', encoding='utf-8'))
